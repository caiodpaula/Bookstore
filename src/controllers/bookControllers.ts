...
const validateBookData = (req: Request, res: Response, next: Function) => {
  ...
  if (!subtitle || typeof subtitle !== "string" || subtitle.trim() === "") {
    return res
      .status(400)
      .json({
        error: 'O campo "subtitle" é obrigatório e deve ser uma string.',
      });
  }
  if (!image || typeof image !== "string" || !image.startsWith("http")) {
    return res
      .status(400)
      .json({
        error: 'O campo "image" é obrigatório e deve ser uma URL válida.',
      });
  }
  if (typeof price !== "number" || price <= 0) {
    return res
      ...
      .json({
        error:
          'O campo "price" é obrigatório e deve ser um número maior que zero.',
      });
  }
